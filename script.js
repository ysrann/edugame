...
musicToggle.style.backgroundColor = "#6a1b9a";
...
function clearPreviousHighlights() {
  document.querySelectorAll(".cell.previous-red, .cell.previous-yellow")
    .forEach(cell => {
      cell.classList.remove("previous-red", "previous-yellow");
    });
}
...
function rollDice() {
  ...
  const oldPos = positions[playerIndex];
  let newPos = oldPos + dice;
  if (newPos > 99) newPos = 99;

  clearPreviousHighlights(); // Hapus highlight sebelumnya

  // Tandai kotak sebelum pindah
  const prevCell = board.querySelector(`.cell[data-index='${oldPos}']`);
  if (prevCell) {
    prevCell.classList.add(playerIndex === 0 ? "previous-red" : "previous-yellow");
  }
...
setTimeout(() => {
  ...
  location.reload();
}, 500); // beri jeda supaya suara terdengar
    }
  });
}
